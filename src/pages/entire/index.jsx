...
export const Entire = () => {
  ...
  const [feedback, setFeedback] = useState(null); // For user messages
  ...
  const onSubmit = (e) => {
    ...
    if (selectedDates.length === 2) {
      ...
      setSelectedDates([]);
      setFeedback({ type: "success", message: "Reservation added!" });
    }
  };
  ...
  return (
    <div className="reservation-container">
      <h1 className="page-title">Reserve Entire Days</h1>
      ...
      <form onSubmit={onSubmit} className="date-range-form" aria-label="Date range reservation form">
        <div className="date-row">
          <label htmlFor="start-date">Start Date:</label>
          <span id="start-date">
            {selectedDates[0] ? selectedDates[0].toLocaleDateString() : <em>Not selected</em>}
          </span>
        </div>
        <div className="date-row">
          <label htmlFor="end-date">End Date:</label>
          <span id="end-date">
            {selectedDates[1] ? selectedDates[1].toLocaleDateString() : <em>Not selected</em>}
          </span>
        </div>
        <button
          type="submit"
